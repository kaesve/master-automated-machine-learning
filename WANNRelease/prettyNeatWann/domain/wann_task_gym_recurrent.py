...
from domain.task_gym_recurrent import RecurrentGymTask
from neat_src import *


class RecurrentWannGymTask(RecurrentGymTask):
  ...
  def __init__(self, game, paramOnly=False, nReps=1): 
    ...
    RecurrentGymTask.__init__(self, game, paramOnly, nReps)
  ...
  def getFitness(self, ind, hyp, \
    ...
    for iRep in range(nRep):
      for iVal in range(nVals):
        ...
        ind.clearState()
        ind.wMultiplier = wVals[iVal]
        if seed == -1:
          reward[iRep,iVal] = self.testInd(ind, seed=seed,view=view)
        else:
          reward[iRep,iVal] = self.testInd(ind, seed=seed+iRep,view=view)
